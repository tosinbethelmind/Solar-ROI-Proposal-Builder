# --------------------------------------------------------------
# Solar ROI Proposal Builder – Lead Status & PDF Export automation (fixed)
# --------------------------------------------------------------
...
# The SUPABASE_PROJECT_REF environment variable must be set before running.

Write-Host "`n=== Applying Supabase migration ===`n"
if (-not $env:SUPABASE_PROJECT_REF) {
    Write-Error "SUPABASE_PROJECT_REF environment variable is not set. Set it and re-run the script."
    exit 1
}
...
Write-Host "`n=== Automation complete! ===`nRun the app (npm run dev) and you should see the new Status column and Export PDF button."
