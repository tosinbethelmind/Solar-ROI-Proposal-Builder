...
export default function ArticleDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Handle async dynamic parameters safely across different Next.js versions
  ...
  if (!article) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 p-6 text-center space-y-4">
        ...
        <Link href="/blog">
          <Button className="bg-teal-650 hover:bg-teal-700 text-white font-bold rounded-xl text-xs">
            Back to Blog Hub
          </Button>
        </Link>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <header className="sticky top-0 z-50 border-b bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto flex h-14 items-center justify-between px-4 sm:px-6">
          <Link href="/blog" className="flex items-center gap-1.5 text-xs font-bold text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-10">
        ...
        <section className="space-y-4 text-center sm:text-left">
          ...
          <h1 className="text-2xl sm:text-4xl font-black tracking-tight leading-tight text-slate-850 dark:text-slate-50">
            {article.title}
          </h1>
          ...
          <div className="flex items-center justify-center sm:justify-start gap-2.5 pt-2 text-xs font-bold text-slate-600 dark:text-slate-400">
            <div className="flex size-7 items-center justify-center rounded-full bg-teal-600 text-white font-black text-xs uppercase">
        ...
        <section className="relative overflow-hidden rounded-3xl border border-teal-500/20 bg-gradient-to-br from-teal-500/5 via-teal-500/10 to-transparent p-6 sm:p-7 shadow-sm">
          <div className="absolute top-0 right-0 w-24 h-24 bg-teal-550/10 rounded-full blur-2xl pointer-events-none" />
          <div className="space-y-3 relative z-10">
            <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-teal-600 text-white shadow-sm">
              <Sparkles className="w-3 h-3" /> Quick Take / RAG Answer
            </div>
            <p className="font-extrabold text-sm sm:text-base leading-relaxed text-slate-850 dark:text-slate-100">
              {article.answerFirst}
            </p>
          </div>
        </section>
        ...
        <section className="space-y-8 prose prose-slate dark:prose-invert max-w-none">
          {article.sections.map((section, idx) => (
            <div key={idx} className="space-y-4">
              <h2 className="text-xl sm:text-2xl font-black tracking-tight text-slate-850 dark:text-slate-50 border-b border-slate-200 dark:border-slate-850 pb-2">
                {section.title}
              </h2>
        ...
        <section className="pt-4">
          {article.widgetType === 'roi-calculator' && (
            <Card className="border-teal-500/25 bg-gradient-to-br from-slate-900 via-teal-950 to-slate-950 text-white shadow-xl rounded-3xl p-6 sm:p-8 space-y-6 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-teal-550/10 rounded-full blur-2xl pointer-events-none" />
              ...
              <div className="pt-2 flex flex-col sm:flex-row gap-3 items-center justify-between">
                ...
                <Button 
                  ...
                  className="bg-teal-650 hover:bg-teal-700 text-white rounded-xl font-bold text-xs px-5 h-9 shrink-0"
                >
          ...
          {article.widgetType === 'grid-vs-solar' && (
            <Card className="border-teal-500/25 bg-gradient-to-br from-slate-900 via-teal-950 to-slate-950 text-white shadow-xl rounded-3xl p-6 sm:p-8 space-y-6 relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-white/5 p-3.5 rounded-2xl border border-white/5 text-center">
                  <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Solar LCOE Average</p>
        ...
        <section className="p-5 bg-gradient-to-r from-teal-650 to-emerald-700 text-white rounded-3xl shadow-lg flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <Button 
            onClick={() => router.push('/estimator')}
            className="bg-white text-teal-950 hover:bg-slate-100 rounded-xl font-black text-xs px-6 h-9 shrink-0 flex items-center gap-1.5"
          >
      ...
      <footer className="border-t border-slate-200 dark:border-slate-800 mt-16 bg-white dark:bg-slate-900 py-10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400 dark:text-slate-500">
          ...
          <p>© {new Date().getFullYear()} SolarPro. Licensed installers in all Lagos DisCos.</p>
        </div>
      </footer>
    </div>
  );
