        ...
        <div className="flex h-16 items-center px-6 gap-2.5 border-b border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50">
          <Link href="/admin" className="flex items-center gap-2.5 group">
            ...
            <div className="flex flex-col">
              <span className="font-extrabold text-base tracking-tight text-slate-855 dark:text-slate-50">SolarQuotePro</span>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 group border ${
                  isActive 
                    ? 'bg-teal-500/10 border-teal-550/20 text-teal-655 dark:bg-teal-950/40 dark:border-teal-950/50 dark:text-teal-400 shadow-sm' 
                    : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-855 dark:hover:text-slate-100 hover:bg-slate-100/50 dark:hover:bg-slate-800/40'
                }`}
              >
                <item.icon className={`h-4 w-4 transition-transform duration-200 group-hover:scale-105 ${
                  isActive ? 'text-teal-600 dark:text-teal-400' : 'text-slate-450 dark:text-slate-500 group-hover:text-slate-70 group-hover:text-slate-700 dark:group-hover:text-slate-350'
                }`} />
                <span>{item.name}</span>
              </Link>
            );
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          <div className="flex items-center justify-between gap-2.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-850 p-3 rounded-2xl shadow-sm">
            <div className="flex flex-col min-w-0">
              <span className="text-[9px] font-bold text-teal-650 dark:text-teal-400 uppercase tracking-wider">Super Operator</span>
              <span className="text-xs font-extrabold text-slate-755 dark:text-slate-300 truncate mt-0.5" title={adminEmail ?? ''}>
                {adminEmail ?? 'Fetching...'}
              </span>
            </div>
            <button 
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col w-64 bg-white dark:bg-slate-900 transition-transform duration-300 ease-in-out border-r border-slate-200 dark:border-slate-800 lg:hidden ${
        ...
        <div className="flex h-16 items-center justify-between px-6 border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-extrabold text-base tracking-tight text-slate-855 dark:text-slate-50">SolarQuotePro Admin</span>
          </div>
          <button 
            onClick={() => setMobileMenuOpen(false)}
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 ${
                  isActive 
                    ? 'bg-teal-500/10 text-teal-655 dark:bg-teal-950/40 dark:text-teal-400 border border-teal-200/50 dark:border-teal-900/30' 
        ...
        <header className="flex h-16 items-center justify-between px-6 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg shrink-0 z-30 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <span className="font-extrabold text-base text-slate-855 dark:text-slate-50 lg:block hidden">Admin Console</span>
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 border-r border-slate-250 dark:border-slate-850 pr-4 lg:flex hidden">
              <ShieldCheck className="h-4 w-4 text-teal-650 dark:text-teal-400 shrink-0" />
