...
export default function InstallerProfileClient({ id }: InstallerProfileClientProps) {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
        ...
        <div className="max-w-7xl mx-auto flex h-14 items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            <Button 
              ...
              size="sm" 
              className="bg-teal-655 text-white hover:bg-teal-700 font-extrabold text-[11px] rounded-xl h-8 border-none"
            >
              Request Quote
            </Button>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        {!installer.is_claimed && (
          <Card className="rounded-3xl border border-amber-500/25 bg-amber-500/5 dark:bg-amber-950/10 p-5 flex flex-col sm:flex-row justify-between items-center gap-4 animate-in slide-in-from-top-4 duration-300">
            <div className="flex items-start gap-3">
              <div className="size-10 rounded-2xl bg-amber-500/10 text-amber-600 dark:bg-amber-950/40 dark:text-amber-400 flex items-center justify-center shrink-0 mt-0.5">
                <AlertTriangle className="w-5 h-5" />
              </div>
              <div className="space-y-1">
                <h3 className="text-sm font-black text-slate-800 dark:text-slate-100">Are you the owner of {installer.business_name}?</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-normal max-w-2xl font-semibold">
                  This profile is a seeded basic listing. Claim it today to update your CAC certifications, manage service coverages, and access the installer dashboard workspace.
                </p>
              </div>
            </div>
            <Button 
              onClick={() => setShowClaimModal(true)}
              className="bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-black text-xs h-9 px-6 border-none shrink-0"
            >
              Claim This Listing ➔
            </Button>
          </Card>
        ...
        <div className="space-y-6 md:col-span-2">
          ...
          <Card className="rounded-3xl border border-slate-200 dark:border-slate-855 bg-white dark:bg-slate-900 p-6 sm:p-8 space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
              ...
              {/* Logo / Initial Avatar */}
              <div className="size-16 rounded-2xl bg-gradient-to-br from-teal-500/20 to-emerald-500/10 flex items-center justify-center text-teal-655 font-black text-2xl border border-teal-500/20 shadow-inner">
            ...
            <div className="space-y-3">
              <h3 className="font-extrabold text-xs sm:text-sm text-slate-855 dark:text-slate-100">About the Installer</h3>
          ...
          <Card className="rounded-3xl border border-slate-200 dark:border-slate-855 bg-white dark:bg-slate-900 p-6 sm:p-8 space-y-6">
            ...
            <div className="relative border-l border-slate-200 dark:border-slate-800 ml-3 pl-6 space-y-6 py-2">
              {[
                { title: "Requirements Evaluation", desc: "Your fuel and NEPA bills are securely forwarded to the partner. They evaluate battery sizing templates." },
        ...
        <div className="space-y-6">
          ...
          <Card className="rounded-3xl border border-slate-200 dark:border-slate-855 bg-white dark:bg-slate-900 p-6 space-y-4 shadow-sm">
            ...
            <Button 
              onClick={() => setShowLeadModal(true)}
              className="w-full bg-gradient-to-r from-teal-655 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white rounded-xl font-black text-xs h-10 border-none"
            >
